...
model = torch.load('/Users/priyapatil/Downloads/basic_cnn_model.pth', map_location=torch.device('cpu'))
model.eval()
...
def capture_frames(stop_event):
    cap = cv2.VideoCapture(0)  # 0 should be the default webcam, use 1 if you have multiple webcams
    if not cap.isOpened():
        st.error("Failed to connect to the webcam.")
        return
